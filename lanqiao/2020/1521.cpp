...
{  // 当前位置i放x字母后，后面的n个位置是否符合要求
    ...
    for (int L = 1; L <= n; L++) {
        ...
        // L-1-cnt[j]是i+1~i+L-1范围内的字母比当前位置L的字母大的字符数量值，也就是
        // i+1~i+L-1区间内字母和L构成的逆序对个数
        ...
        for (int j = 26; j >= 1; j--) {
            ...
            if (L - 1 - cnt[j] + num > ma) {
                // - cnt[j]
                // 如果在i+1到当前位置i+L前有放过字母j，然后当前位置i+L也尝试放j，
                // 那当前位置不和前面放j的位置构成逆序对 要减掉前面放j的位置个数
                // i+1到i+L-1的字母一定是逆序排序的，因为这样排得到的ma最大
                // 不会出现azz这种情况
            ...
            // 因为j是从26逆着放到1的
            // 到下一轮循环时，j--假设记为j'，那要求的1-i里比j'大的数就是num
...
signed main()
{
    ...
    while (T--) {
        ...
        for (int i = 1; i <= len; i++) {  // 按长度从左往右尝试放字母
            for (int j = 1; j <= 26;
                 j++) {  // 按顺序'a'-'z' 26个字母放   先用1-26代表这26个字母
                if (check(j, len - i)) {  // 现在是在第i位放j字母
                                          // 看i后面的len-i个字母能否符合条件
